...
#     """Convert PIL image to base64 data URL with compression."""
#     buf = BytesIO()
#     img = img.resize((int(img.width * 0.75), int(img.height * 0.75)))  # Compress for efficiency
...
#     images = []
#     zoom = 3.0  # Increased zoom for better OCR accuracy
...
#     """
#     Send one page image (as data URL) to OpenAI Vision model and get plain text back, with retries.
#     """
#     max_retries = 3
...
#             print(f"Vision API attempt {attempt + 1} failed: {str(e)}")
...
#             print(f"Extracted content for {resume.filename}: {content[:500]}...")  # Log for debugging
#         except Exception as e:
...
#                 Return the output in JSON format. If a field cannot be extracted or the resume text is empty or 'No content extracted', use null or an empty list as appropriate. Always ensure the output is valid JSON.
#                 Resume text:
#                 {resume_text}
#                 """
...
#                     model="gpt-4o-mini",
#                     messages=[
#                         {"role": "system", "content": "You are a highly accurate assistant skilled in extracting structured data from resume text."},
#                         {"role": "user", "content": prompt.format(resume_text=content or "No content extracted")}
#                     ],
#                     response_format={"type": "json_object"}
#                 )
#                 raw_content = response.choices[0].message.content
#                 print(f"Raw OpenAI response for {resume.filename}: {raw_content}")
#                 extracted_data = json.loads(raw_content)
#                 break
#             except json.JSONDecodeError as json_err:
#                 print(f"Invalid JSON from OpenAI for {resume.filename}: {str(json_err)} - Raw content: {raw_content if 'raw_content' in locals() else 'No content'}")
#                 if attempt < max_retries - 1:
...
#                 }
#             except Exception as e:
#                 print(f"OpenAI extraction attempt {attempt + 1} failed for {resume.filename}: {str(e)}")
#                 if attempt < max_retries - 1:
#                     time.sleep(2 ** attempt)
#                 else:
#                     extracted_data = {
...
#             raise HTTPException(status_code=500, detail=f"Failed to extract data from {resume.filename} after retries")
...
def ocr_page_with_openai_dataurl(data_url: str, model: str = "gpt-4o-mini", max_tokens: int = 2000) -> str:
    """
    Send one page image (as data URL) to OpenAI Vision model and get plain text back, with retries.
    """
    max_retries = 3
...
def extract_text_from_scanned_pdf(file):
    ...
    try:
        ...
        for i, img in enumerate(pages, start=1):
            data_url = pil_to_data_url(img, fmt="PNG")
            page_text = ocr_page_with_openai_dataurl(data_url, model="gpt-4o-mini", max_tokens=2000)
            if page_text and page_text.strip():
                content += f"=== Page {i} ===\n{page_text}\n\n"
                print(f"Extracted text (Vision API) for page {i}: {page_text[:50]}...")
            time.sleep(1)  # Increased delay to avoid rate limits
    ...
    for i, resume in enumerate(resumes):
        ...
        for attempt in range(max_retries):
            try:
                ...
                Return the output as a valid JSON object only, without any markdown formatting (e.g., no ```json
                Resume text:
                {resume_text}
                """
                response = openai_client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are a highly accurate assistant skilled in extracting structured data from resume text. Ensure the response is a valid JSON object without any markdown or extra text."},
                        {"role": "user", "content": prompt.format(resume_text=content or "No content extracted")}
                    ],
                    response_format={"type": "json_object"}
                )
                ...
                print(f"Invalid JSON or format from OpenAI for {resume.filename}: {str(e)} - Raw content: {raw_content}")
                if attempt < max_retries - 1:
                    time.sleep(2 ** attempt)
                    continue
                extracted_data = {
